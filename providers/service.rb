...
action :install do
  if (@current_resource.installed_already)
...
def load_current_resource
  @current_resource = Chef::Resource::MysqlWindowsService.new(@new_resource.name)
  @current_resource.installation_name(new_resource.installation_name)
  @current_resource.msi_url_path(new_resource.msi_url_path)
  @current_resource.msi_url_file(new_resource.msi_url_file)
  @current_resource.installation_path(new_resource.installation_path)
  @current_resource.remove_completely(new_resource.remove_completely)
  @current_resource.installed_already = InstallationCheck.installed? 'mysql', :lower
